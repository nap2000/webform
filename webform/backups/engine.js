...
//			},
            ...
            distance : {
                ...
                fn: function(a, b)      // smap - add opion of a second argument
                {
                    var	geopoints = [];
                ...
                args: [
                    {t: 'string'},
                    {t: 'string', r: false}     // smap add optional second argument
                ],
             ...
             * Smap Functions
             *
             * Take a URL for a media item and use it to populate a question's default value
             * arguments provided.
             *
             * Convert to openrosa xpath
             * --------------------------
             * x.toNodeSet()  => x.v
             * x.toString() => asString(x)
             * StringType(x) => XPR.string(x)
             * nodeset.value[0] => nodeset.v[0].innerHTML
             *
             *
             *
             * @param {NodesetType} url
             * @param {StringType} image
             * @return {StringType}
             */
            get_media : {

                fn: function(url, image)
                {
...
>>>>>>> production
                    if(inputElem) {
                        ...
                        alert("missing second parameter that idenifies the image question")
                    }
            ...
            lookup_image_labels : {
                ...
                fn: function(image)
                {
                    ...
                    if ( image ) {
                        let imagePath = getPath(image.value[0]);
                    ...
                    return new StringType(window.wfcache[key] ? window.wfcache[key] : "");
                },
            ...
            lookup : {
                ...
                fn: function(source, data_column, key_column, key_value, index, searchType) {
                    ...
                    if(key_value && key_value.toString().length > 0) {
                        var url = window.location.protocol + '//' + window.location.hostname;
                            ...
                            if (!window.wfcache[url]) {
                                ...
                                xmlHttp.onreadystatechange = function () {
                                    ...
                                    if (xmlHttp.readyState == 4 && xmlHttp.status == 200) {
                                        ...
                                        if ( sourceNode ) {
                                            let sourcePath = getPath(sourceNode.value[0]);
                    ...
                    if(window.wfcache[url]) {
                        ...
                        if(searchType && index == -1) {
                            return new StringType(obj["_count"]);
                        } else {
                            return new StringType(obj[data_column.toString()]);
                        }
                    } else {
                        return new StringType("");
                    }
            ...
            lookup_choices : {
                ...
                fn: function(source, p1, p2, p3, p4, p5, p6) {

                    /*
					 * Process parameters
					 */
                    var question = arguments[arguments.length - 1].toString();
                    var func = arguments.length > 2 ? p1.toString() : undefined;
                    var exp = (func == 'eval' ? p2.toString() : undefined);
                    var hasFilter1 = func && !exp && arguments.length > 4;
                    var f1_column = hasFilter1 ? p2.toString() : undefined;
                    var f1_value = hasFilter1 ? p3.toString() : undefined;
                    var hasFilter2 = arguments.length > 6;
                    var f2_column = hasFilter2 ? p4.toString() : undefined;
                    var f2_value = hasFilter2 ? p5.toString() : undefined;
                    ...
                    let calcPath = getPath(this.node);
            ...
            default : {
                ...
                fn: function(a)
                {
                    ...
                    return new StringType(newValue);
                },
    ...
    var getChildByClass = function(b, c) {
        if(b && b.childNodes) {
