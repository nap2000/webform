  ...
  const func = {
    ...
    lookup: function(source, data_column, key_column, key_value, index, searchType) {
      ...
      if (searchType) {
        ...
        url += "/" + encodeURIComponent(kv);
        url += '?index=' + asString(index);
        url += '&searchType=' + asString(searchType);
      } else if (index) {
        // 5 parameter version
        url += "/" + null;
        url += "/" + null;
        url += '?index=' + asString(key_value);
        url += '&searchType=eval';
      ...
      if (url) {
          ...
          if (!window.wfcache[url]) {
            ...
            xmlHttp.onreadystatechange = function () {
              ...
              if (xmlHttp.readyState == 4 && xmlHttp.status == 200) {
                ...
                if (theSearchType || (sourceNode && !theIndex)) { // 6 and 4 parameters
        ...
        if (window.wfcache[url]) {
          ...
          if ((searchType && index && asString(index) === 'count')
              || (index && asString(index) === 'eval' && asString(key_value) === 'count')) {
        ...
        if(lhs.t === 'date' || rhs.t === 'date') {
          ...
          if(op >= EQ && op <= GTE) {
            ...
            //if(lhs.t === 'date' && rhs.t === 'date') err('No handling for simple arithmetic with two dates.');
              let res;  // smap
              if(lhs.t === 'date' && rhs.t === 'date') { // smap add date arithmetic
                  const d1 = lhs.v;
                  const d2 = rhs.v;
                  if(d1.toString() !== 'InvalidDate' && d1.toString() !== 'InvalidDate') {
                      let n = d2.getTime();
                      if (op === MINUS) n = -n;
                      res = (d1.getTime() + n) / (1000*60*60*24);   // days
                  } else {
                      res = d1; // Invalid
                  }
              } else {
    ...
    case 'str':
      ...
      if(RAW_NUMBER.test(r)) {
        ...
        if(temp !== -1) { // smap
            ...
            timePart = timePartSplit[0];
            if(timePart[0] === 'T') {
                timePart = timePart.substring(1);
            }
            timePart = timePart.split(':');
            let timePartTemp = 'T' + _zeroPad(timePart[0]);
            timePartTemp += ':';
            if(timePart.length > 0) {
                timePartTemp += _zeroPad(timePart[1]);
            } else {
                timePartTemp += '00';
            }
            timePartTemp += ':';
            ...
            if(timePartTemp) {
                timePart = timePartTemp;
            } else {
                timePart = 'T00:00:00.000';
            }
        } else {
            timePart = 'T00:00:00.000';
        }
        ...
        if(isValidDate(temp[0], temp[1], temp[2])) {
          const time = `${_zeroPad(temp[0])}-${_zeroPad(temp[1])}-${_zeroPad(temp[2])}`+
            ...
            timePart + (new Date(r)).getTimezoneOffsetAsTime();  // smap
