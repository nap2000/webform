...
//			},
			...
			lookup : {
				...
				fn: function(source, data_column, key_column, key_value) {
			...
			lookup_choices : {
				...
				fn: function(source, p1, p2, p3, p4, p5, p6) {
					...
					let selectPath = calcPath.replace('__dynamic', '');
					...
					if(exp) {
						exp = expression.value.replaceAll("##", "'");
						...
						if (!window.wfcache[url]) {
							...
							xmlHttp.onreadystatechange = function () {
								...
								if (xmlHttp.readyState == 4 && xmlHttp.status == 200) {
									window.wfcache[theUrl] = xmlHttp.responseText;
									window.wfcache.ongoing[theUrl] = undefined;
									updateChoiceList(xmlHttp.responseText, selectPath, lang, question);
									console.log("got response: " + url);
								} else if (xmlHttp.readyState == 4) {
									alert(xmlHttp.responseText);
									window.wfcache[theUrl] = "";
									window.wfcache.ongoing[theUrl] = undefined;
									updateChoiceList("", selectPath, lang, question);
								}
							...
							updateChoiceList(window.wfcache[url], selectPath, lang, question);
							console.log("using cache: " + url);
						}
					}
		...
		if(input) {
			...
			for (i = 0; i < eList.length; i++) {
				...
				if (s.startsWith("/main")) {
					elem = document.getElementsByName(s)[0];
					if(elem) {
	...
	var updateChoiceList = function(responseText, selectPath, lang, question) {
		var i;
		var jsonResp = JSON.parse(responseText);
		var html = genOptionsHtml(jsonResp, selectPath, lang);
	...
	var genOptionsHtml = function(options, path, lang) {
		var h = [],
			idx = -1,
			i;
		if(options && options.length > 0) {
			for(i = 0; i < options.length; i++) {
				...
				h[++idx] = '<input type="radio" data-type-xml="string" name="';
				h[++idx] = path;
				h[++idx] = '" data-name="';
				h[++idx] = path;
				h[++idx] = '" value="';
